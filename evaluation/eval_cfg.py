...
if __name__ == '__main__':
    ...
    colmap_folders = glob(os.path.join(image_path, '**/colmap'), recursive=True)
    ...
    df.to_csv(os.path.join(image_path, 'eval.csv'), float_format='%.4f', index=False)
    print('save to', os.path.join(image_path, 'eval.csv'))
    ...
    stats.to_csv(os.path.join(image_path, 'eval_stats.csv'), float_format='%.4f', index=True)
