...
def save(
    ...
    if int(global_rank) == 0:
        ...
        torch.save(ema.state_dict(), os.path.join(root_save_dir, "ema.pt"))
        model_sharding(ema)
        if camera_encoder is not None:
            torch.save(camera_encoder.state_dict(), os.path.join(root_save_dir, "camera.pt"))
    ...
    dist.barrier()
    return save_dir if int(global_rank) == 0 else None
