...
def resize_scale(clip, target_size, interpolation_mode):
    ...
    rescaled_size = [round(H * scale_), round(W * scale_)]

    return torch.nn.functional.interpolate(clip, size=rescaled_size, mode=interpolation_mode, align_corners=False)
...
def center_crop(clip, crop_size):
    ...
    th, tw = crop_size
    if h < th or w < tw:
        raise ValueError(f"height and width {h}x{w} must be no smaller than crop_size {th}x{tw}")
