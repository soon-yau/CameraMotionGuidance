...
class STDiTBlock(nn.Module):
    def __init__(
        ...
        camera_fuser_linear_dims=[],
        camera_format: Literal['extrinsic','plucker']='extrinsic'
    ):
        super().__init__()
        ...
        self.camera_fuser = nn.Linear(*camera_fuser_linear_dims) if len(camera_fuser_linear_dims) == 2 else None
        self.camera_format = camera_format

    def forward(self, x, y, t, mask=None, tpe=None, camera=None):
        ...
        if self.camera_format == 'plucker':
            camera = rearrange(camera, 'B T C H W -> (B H W) T C')
            x_t += camera
            
        elif self.camera_format == 'extrinsic' and self.camera_fuser is not None:
            # Repeat [B, T, 12] -> [B, T, S, 12]
            camera = camera.unsqueeze(2).repeat_interleave(repeats=self.d_s, dim=2)
            camera = rearrange(camera, "B T S C-> (B S) T C", T=self.d_t, S=self.d_s)
            x_t = torch.cat((x_t, camera), -1)
            assert self.camera_fuser is not None

        if self.camera_fuser is not None:
            x_t = self.camera_fuser(x_t)
...
class STDiT(nn.Module):
        ...
        self.blocks = nn.ModuleList(
            [
                STDiTBlock(
                    ...
                    camera_fuser_linear_dims=camera_fuser_linear_dims,
    ...
    def forward(self, x, timestep, y, mask=None, camera=None): #TODO ADD CONDITION
        """
        ...
        y = y.to(self.dtype)
        camera = camera.to(self.dtype) if camera is not None else None
        # embedding
        ...
        for i, block in enumerate(self.blocks):
            if i == 0:
                ...
                tpe = None
            x = auto_grad_checkpoint(block, x, y, t0, y_lens, tpe, camera)
    ...
    def initialize_weights(self):
        ...
        for block in self.blocks:
            ...
            #block.camera_fuser._initialize_()
            if block.camera_fuser:
                nn.init.eye_(block.camera_fuser.weight)
                nn.init.zeros_(block.camera_fuser.bias)
