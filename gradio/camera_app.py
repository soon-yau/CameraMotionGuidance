...
from opensora.datasets.dataset_realestate import DatasetFromImages

cache_dir = Path('_gradio_cache')
N_REF_MOTIONS = 3
...
# Load reference
dataset = DatasetFromImages(video_len=16)
...
def load_ref(*ref_videos):
    
    global dl_iter
    video_files = []

    for i, v in enumerate(ref_videos):
        x = next(dl_iter)
        subdir = Path(cache_dir)/f'ref_{i}'
        os.makedirs(subdir, exist_ok=True)

        # Video
        images = x['images'].squeeze()
        video_file = str(subdir/"ref.mp4")
        video_files.append(video_file)
        write_video(video_file, images, fps=8, video_codec='h264') 

        # Camera
        with open(subdir/"camera.p", "wb") as f:  # Open file in binary write mode
            pickle.dump(np.array(x['camera']), f)  # Use pickle to serialize the array

        with open(subdir/"text.txt", "w") as file:  
            file.write(x['text'][0])  # Write the string to the file
    return video_files


@spaces.GPU(duration=200)
def run_inference(prompt_text, index):
    print('index', index)
    ...
    filename = f"{args.output}/sample_{index}"
...
def main():
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            with gr.Column():
                prompt_text = gr.Textbox(show_label=False, placeholder="Describe your video here", lines=2)
                with gr.Row():
                    #submit_button = gr.Button("Generate video")
                    load_button = gr.Button("Load Next Ref Videos")

        ref_videos = []
        output_videos = []
        submit_buttons = []

        video_kwargs = {'height':384, 'width':384, 'autoplay':True}
        for ref_id in range(N_REF_MOTIONS):
            with gr.Row():
                with gr.Column():
                    ref_video = gr.Video(value=cache_dir/f'ref_{ref_id}/ref.mp4', 
                                         label='motion reference', interactive=False, **video_kwargs)
                    ref_videos.append(ref_video)
                    submit_buttons.append(gr.Button("Generate video"))

                with gr.Column():
                    output_video = gr.Video(label='generated', show_download_button=True, **video_kwargs)
                    output_videos.append(output_video)

        #submit_button.click(fn=run_inference, inputs=[prompt_text], outputs=[*ref_videos])
        load_button.click(fn=load_ref, inputs=[*ref_videos], outputs=[*ref_videos])
        for i, submit_button in enumerate(submit_buttons):
            submit_button.click(fn=run_inference, inputs=[prompt_text, gr.Number(value=i, visible=False)], outputs=output_videos[i])
